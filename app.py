...
<br><br>
Predictions should be considered as preliminary screening results and validated experimentally.
</div>
""", unsafe_allow_html=True)
...
def predict_antibacterial_activity(smiles):
    mol, descriptor_df = calculate_descriptors(smiles)
    ...
    return result, descriptor_df, mol
...
if st.button("Predict"):
    ...
    try:
        result, descriptor_df, mol = predict_antibacterial_activity(smiles)
        ...
        with col2:
            ...
            if result["Prediction"] == "Active":
                st.success("Predicted Activity: Active")
            else:
                st.error("Predicted Activity: Inactive")
        ...
        if "SMILES" not in input_df.columns:
            ...
            if st.button("Run Batch Prediction"):
                ...
                for index, row in input_df.iterrows():
                    ...
                    try:
                        result, descriptor_df, mol = predict_antibacterial_activity(
                            smiles_value
                        )
